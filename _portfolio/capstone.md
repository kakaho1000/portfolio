---
title: "Capstone 6DOF Prosthetic Hand"
header:
    teaser: /assets/images/capstone3.jpg
excerpt: "Electrical integration project adapting an industrial robotic hand for prosthetic use"
date_text: "Apr 2026"
date: 2026-04-07
categories:
  - Class Project
  - Complete
tags:
  - Consultation
  - Onshape
  - Electrical Troubleshooting

---

![image-right]({{ site.url }}{{ site.baseurl }}/portfolio/assets/images/capstone3.jpg){: .align-right}

### Goal 
- Integrate a 24V robotic hand with 5V-7.2V myoelectric prosthetic parts
- Allow passersby to operate the device using muscle signals at the Design & Innovation Day demo

### Personal responsibilities
- Led consultations with local prosthetics clinic to gain insight on user needs and current prosthetic technology 
- Led hardware validation and troubleshooting in an electronics laboratory, reporting to client through video calls
- Designed and 3D printed hand mount for demo

### Results
- Fully integrated system with custom slip-ring PCB, voltage step-down power delivery, control algorithm and LCD display
- 100% of interested passersby successfully activated the device
- Client was very satisfied with the delivered product

<figure class="half">
    <a href="/portfolio/assets/images/capstonetesting.png" title="Capstone Testing Setup"><img src="/portfolio/assets/images/capstonetesting.png"></a>
    <a href="/portfolio/assets/images/capstone2.jpg" title="Device Before Assembly"><img src="/portfolio/assets/images/capstone2.jpg"></a>
</figure>


<video width="800" height = "600" controls>
  <source src="/portfolio/assets/images/capstonevid.mp4" type="video/mp4">
  Your browser does not support the video tag. 
</video>
